...
#Compute individual weight by adding maximum of R and L
                    sampleweight_max[i]=round(min(float(max(sampleweight_L[i])), float(max(sampleweight_R[i])))*2,3)
...
#Designate output 'variantfile' name
    ...
    variantfile.write('CHROM'+'\t'+'POS'+'\t'+'ID:ALT'+'\t'+'REF'+'\t'+'ALT'+'\t'+'WEIGHT'+'\t'+'AltAF'+'\t'+'GENE'+'\t'+'HET_SAMPLEIDs'+'\t'+'HOM_SAMPLEIDs'+'\n')
#Read in all lines of sorted intermediate gzip file
    lines=infile.readlines()
...
#Set genename in line as gene
        gene=words[7]
        numwords=len(words)
        if float(w[words[2]]) > 0:
#Set variantsampleID list with variantfile columns: CHROM POS ID:ALT WEIGHT REF ALT  GENE
            variantsampleID=[words[0], words[1], words[2], words[3], words[4], w[words[2]], words[6], words[7]]
#Open countfile
